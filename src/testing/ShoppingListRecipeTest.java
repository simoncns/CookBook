...
class ShoppingListRecipeTest
{
  ...
  {
    ShoppingListRecipe list = new ShoppingListRecipe();
  }

  @Test
  void constructorTest()
  {
    Recipe r = new Recipe("Test", 10, constructIngredients(), constructUtensils(),
        constructSteps());
    ShoppingListRecipe list = new ShoppingListRecipe(r);
    assertEquals(10, list.getPeople());
    assertEquals("Test", list.getName());
    list.getShoppingListItems();
    list.toStringPrintHelp();
    assertEquals(10, list.getPeople());
    list.toString();
  }
  ...
  void sortTest()
  {
    ...
    ShoppingListRecipe list = new ShoppingListRecipe(r);
    assertEquals(10, list.getPeople());
    assertEquals("Test", list.getRecipe().getName());
    assertEquals("Test", list.getName());
  }

  @Test
  void shoppingListStringErrorTest()
  {
    ShoppingListRecipe list = new ShoppingListRecipe();
    list.shoppingListString();
    Recipe r = new Recipe("Test", -1, constructIngredients(), constructUtensils(),
        constructSteps());
    list = new ShoppingListRecipe(r);
  }
